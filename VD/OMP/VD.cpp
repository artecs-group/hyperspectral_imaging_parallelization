...
map(to: meanSpect[0:bands]) \
map(to: Cov[0:bands*bands]) \
map(to: Corr[0:bands*bands]) \
map(to: CovEigVal[0:bands]) \
map(to: CorrEigVal[0:bands]) \
map(to: U[0:bands*bands]) \
map(to: VT[0:bands*bands]) \
map(to: image[0:lines*samples*bands]) \
map(to: info) \
map(to: rwork) \
map(to: mean[0:bands]) \
map(from: count[0:FPS])
{
    ...
    #pragma omp target data use_device_ptr(image, Cov) device(deviceNum)
    {
    ...
    #pragma omp target data use_device_ptr(Cov, CovEigVal, U, VT, info, rwork) device(deviceNum)
    {
    ...
    #pragma omp target data use_device_ptr(Corr, CorrEigVal, U, VT, info, rwork) device(deviceNum)
    {
    ...
    #pragma omp target device(deviceNum)
    #pragma omp single
