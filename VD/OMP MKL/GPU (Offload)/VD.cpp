...
int main(int argc, char* argv[])
{
    ...
    int initial_dev = omp_get_initial_device(); // This is the hosts number, acts as an id (returns 1)
    int default_dev = omp_get_default_device(); // This is the default device's id (returns 0, which surely refers to the gen9 GPU)
    int num_devices = omp_get_num_devices(); // Returns number of devices other than the host (returns only 1, probably the gen9 GPU)

    printf("initial_dev (host) = %d, default_dev (device) = %d, num_devices = %d\n", initial_dev, default_dev, num_devices);
    ...
    map(tofrom: meanSpect[0:bands]) \
    map(tofrom: Cov[0:bands*bands]) \
    map(tofrom: Corr[0:bands*bands]) \
    map(tofrom: CovEigVal[0:bands]) \
    map(tofrom: CorrEigVal[0:bands]) \
    map(tofrom: U[0:bands*bands]) \
    map(tofrom: VT[0:bands*bands]) \
    map(tofrom: image[0:lines*samples*bands]) \
    map(tofrom: count[0:FPS]) device(default_dev)
    {
	...
	printf("VD algorithm:\t%.5f seconds\n", secsVD);
