...
map(to: rank)
{
	...
	#pragma omp target data use_device_ptr(R_o,svdMat)
	{
	...
	#pragma omp target data use_device_ptr(R_o,Ud,x_p)
	{
	...
	{
		#pragma omp target data use_device_ptr(image_vector,svdMat) 
		{
		...
		#pragma omp target data use_device_ptr(image_vector,Ud,x_p)
		{
	...
	#pragma omp target map(A[:targets*targets]) 
	{
	...
	for(i=0; i<targets; i++)
	{
    	...
    	#pragma omp target update to(tolerance, rank) 

    	#pragma omp target teams distribute map(tolerance, rank) reduction(+: rank) 
		...
		#pragma omp target data use_device_ptr(A2,VT_pinv,Utranstmp)
		{
		...
		#pragma omp target teams distribute map(sum1) reduction(+: sum1)
		for(j=0; j<targets; j++)
		{
			f[j] = w[j] - f[j];
			sum1 += pow(f[j],2);
		...
		#pragma omp target teams distribute parallel for map(sum1)
		...
		#pragma omp target update from(sumxu[:lines_samples], index[:targets], sum2) 
		for(j=0; j<lines_samples; j++)
		{
    	...
    	#pragma omp target update to(sum2, sumxu[:lines_samples], index[:targets]) 
