...
int main(int argc, char *argv[])//(double *image, int lines, int samples, int bands, int targets, double SNR, double *endmembers)
{
	...
	double *S_pinv = (double*) calloc(targets,sizeof(double)); //eigenvalues // A LO MEJOR SE PUEDE HACER ALLOC EN VEZ DE COPYIN
	double *U_pinv = (double*) calloc(targets * targets,sizeof(double)); //eigenvectors
	double *VT_pinv = (double*) calloc(targets * targets,sizeof(double)); //eigenvectors

	double * Utranstmp = (double*)calloc(targets * targets,sizeof(double)); // No hacer copyin, sino create, alloc o como se llame

	//Inicializamos endmembers
	...
	for(i=0; i<targets; i++)
	{
		...
		*/
		...
		work_pinv = (double*)malloc(lwork_pinv*sizeof(double));
		dgesvd_("S", "S",&targets, &targets, A2, &targets, S_pinv, U_pinv, &targets, VT_pinv, &targets, work_pinv, &lwork_pinv, &info_pinv);
    	...
    	dgemm_("T", "N", &targets, &targets, &targets, &alpha, VT_pinv, &targets, Utranstmp, &targets, &beta, A2, &targets);
